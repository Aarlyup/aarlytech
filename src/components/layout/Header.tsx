...
const Header: React.FC = () => {
  ...
  const handleAuthClick = () => {
    ...
    // Direct to auth page for Get Started button
    navigate('/auth');
  };
      ...
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          ...
          {isAuthenticated ? (
            ...
            /* Show Home link on funding, dashboard, and auth pages, Get Started on others */
            location.pathname.startsWith('/funding') || location.pathname.startsWith('/dashboard') || isAuthPage ? (
              <Link
                to="/"
                className="flex items-center space-x-1 bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors font-medium"
                onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
              >
                <Home size={18} />
                <span>Home</span>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-gray-900 border-t border-gray-800">
          <div className="px-6 py-4 space-y-4">
            {/* Always show main navigation for all users */}
            {isAuthenticated && (
            ...
            {isAuthenticated ? (
              ...
              location.pathname.startsWith('/funding') || location.pathname.startsWith('/dashboard') || isAuthPage ? (
                <Link
                  to="/"
                  className="block text-center bg-gray-600 hover:bg-gray-700 text-white font-medium py-3 px-4 rounded-lg transition-colors"
                  onClick={() => { setMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                >
                  Home
                </Link>
