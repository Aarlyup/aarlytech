...
import { AFFILIATIONS, APPLICATION_STATUS, STATES, emptyFilters } from '../../constants/filters';
import { hasActiveIncubatorFilters, hasActiveIncubatorSearch, parseIncubatorQueryToFilters, incubatorFiltersToQuery } from '../../utils/incubatorFilter';

interface IncubatorFiltersProps {
  onFilterChange: (filters: IncubatorFilters) => void;
}
...
const IncubatorFilters: React.FC<IncubatorFiltersProps> = ({ onFilterChange }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeDropdown, setActiveDropdown] = useState<string | null>(null);
  const [stagedFilters, setStagedFilters] = useState<IncubatorFilters>({ ...emptyFilters });
  const [appliedFilters, setAppliedFilters] = useState<IncubatorFilters>({ ...emptyFilters });
  ...
  useEffect(() => {
    ...
    const queryFilters = parseIncubatorQueryToFilters(Object.fromEntries(params));
  ...
  const updateUrlAndState = (newFilters: IncubatorFilters) => {
    const query = incubatorFiltersToQuery(newFilters);
  ...
  const toggleStagedFilter = (category: keyof IncubatorFilters, value: string) => {
  ...
  const removeFilter = (category: keyof IncubatorFilters, value: string) => {
    ...
    <div ref={filterRef} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {hasActiveIncubatorSearch(appliedFilters) && (
            <button
              onClick={clearSearch}
          ...
          {hasActiveIncubatorFilters(appliedFilters) && (
            <button
          ...
          {(hasActiveIncubatorSearch(appliedFilters) || hasActiveIncubatorFilters(appliedFilters)) && (
            <button
              onClick={resetAll}
              className="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-1"
            >
      ...
      {(hasActiveIncubatorSearch(appliedFilters) || hasActiveIncubatorFilters(appliedFilters)) && (
        <div className="mt-4 pt-4 border-t border-gray-100">
          ...
          <FilterChips filters={appliedFilters} onRemove={removeFilter} />
        </div>
      )}
    </div>
  );
...
export default IncubatorFilters; 